...
class SwiftHelpersTests: XCTestCase {
    ...
    func testIsPath_absoluteURL() {
        self.testIsPath("/foo/bar/baz", isAbsoluteMatcher: true)
    }

    func testIsPath_relativeURL() {
        self.testIsPath("foo/bar/baz", isAbsoluteMatcher: false)
    }
    ...
    func testPathStartsWith_absoluteURL() {
        self.testPathStartsWith("/foo/bar", isAbsoluteMatcher: true)
    }

    func testPathStartsWith_relativeURL() {
        self.testPathStartsWith("foo/bar", isAbsoluteMatcher: false)
    }
    ...
    func testPathMatches_caseSensitive() {
        self.testPathMatches("/bar/[0-9]+/baz$", caseInsensitive: false)
    }

    func testPathMatches_casensensitive() {
        self.testPathMatches("/bar/[0-9]+/baz$", caseInsensitive: true)
    }
    ...
    let trueMatcher: OHHTTPStubsTestBlock = { _ in true }
    let falseMatcher: OHHTTPStubsTestBlock = { _ in false }

    func testOrOperator() {
        for url in self.sampleURLs {
            #if swift(>=3.0)
                let req = URLRequest(url: URL(string: url)!)
            #else
                let req = NSURLRequest(URL: NSURL(string: url)!)
            ...
            XCTAssert((self.trueMatcher || self.falseMatcher)(req) == true, "trueMatcher || falseMatcher should result in a trueMatcher")
            XCTAssert((self.falseMatcher || self.trueMatcher)(req) == true, "falseMatcher || trueMatcher should result in a trueMatcher")
            XCTAssert((self.falseMatcher || self.falseMatcher)(req) == false, "falseMatcher || falseMatcher should result in a falseMatcher")
        }
    }
    ...
    func testAndOperator() {
        for url in self.sampleURLs {
            ...
            #endif
            XCTAssert((trueMatcher && trueMatcher)(req) == true, "trueMatcher && trueMatcher should result in a trueMatcher")
            XCTAssert((self.trueMatcher && self.falseMatcher)(req) == false, "trueMatcher && falseMatcher should result in a falseMatcher")
            XCTAssert((self.falseMatcher && self.trueMatcher)(req) == false, "falseMatcher && trueMatcher should result in a falseMatcher")
            XCTAssert((self.falseMatcher && self.falseMatcher)(req) == false, "falseMatcher && falseMatcher should result in a falseMatcher")
        }
    }
    ...
    func testNotOperator() {
        for url in self.sampleURLs {
            ...
            XCTAssert((!self.falseMatcher)(req) == true, "!falseMatcher should result in a trueMatcher")
        }
    }
}
