...
extension PaymentSession: Decodable {
    ...
    internal static func decode(from responseString: String) throws -> PaymentSession {
        ...
        guard let json = try JSONSerialization.jsonObject(with: Data(responseString.utf8), options: []) as? [String: Any] else { throw self.decodingError }
        guard let paymentSessionBase64 = json["paymentSession"] as? String else { throw self.decodingError }
        guard let paymentSessionData = Data(base64Encoded: paymentSessionBase64) else { throw self.decodingError }
    ...
    private mutating func fillInLogoURLs() {
        paymentMethods.preferred = self.paymentMethodsWithLogoURLs(paymentMethods.preferred)
        paymentMethods.other = self.paymentMethodsWithLogoURLs(paymentMethods.other)
    }
