    ...
    struct Surcharge {
        ...
        public var formatted: String {
            ...
            if let fixed = fixedCost, fixed != 0 {
                let formattedAmount = AmountFormatter.formatted(amount: fixed, currencyCode: self.currencyCode) ?? String(fixed) + " " + self.currencyCode
...
extension PaymentMethod: Hashable {
    ...
    public var hashValue: Int {
        return self.type.hashValue ^ self.name.hashValue ^ self.paymentMethodData.hashValue
    }

    public func hash(into hasher: inout Hasher) {
        hasher.combine(self.type)
        hasher.combine(self.name)
        hasher.combine(self.paymentMethodData)
    }
...
public extension PaymentMethod {
    ...
    var accessibilityLabel: String {
        ...
        switch storedDetails {
        case let storedDetails as StoredCardPaymentDetails:
            return ADYLocalizedString("creditCard.stored.accessibilityLabel", self.name, storedDetails.number)
        case let storedDetails as StoredPayPalPaymentDetails:
            return "\(self.name) \(storedDetails.emailAddress)"
        default:
            return self.name
        }
    }
