...
class GiroPayFormViewController: FormViewController {
    ...
    override func viewDidLoad() {
        ...
        title = self.paymentMethod.name
        formView.addFormElement(self.searchFormSection)
    }
    ...
    private func didSelect(issuer: GiroPayIssuer) {
        ...
        self.issuerView.title = issuer.bankName
        self.issuerView.subtitle = issuer.bic + " / " + issuer.blz
        self.issuerView.isHidden = false
        self.searchNavigationController.dismiss(animated: true)
        formView.payButton.isEnabled = true
    }

    private func didRemoveIssuerSelection() {
        self.issuerView.isHidden = true
...
extension GiroPayFormViewController: GiroPaySearchControllerDelegate {
    func didStartNetworkRequest() {
        self.issuersTableViewController.isLoading = true
    }

    func didFinishNetworkRequest() {
        self.issuersTableViewController.isLoading = false
    }

    func didFail(with error: Error) {
        self.issuersTableViewController.error = error
    }

    func didUpdate(with issuers: [GiroPayIssuer]) {
        self.issuersTableViewController.issuers = issuers
    }
}

extension GiroPayFormViewController: UISearchResultsUpdating, UISearchBarDelegate {
    func updateSearchResults(for searchController: UISearchController) {
        self.issuerSearchController.searchString = searchController.searchBar.text ?? ""
    }

    func searchBarCancelButtonClicked(_: UISearchBar) {
        self.searchNavigationController.dismiss(animated: true, completion: nil)
    }
}
