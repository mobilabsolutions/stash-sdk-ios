...
open class FormSection: UIStackView {
    public init() {
        ...
        layoutMargins = UIEdgeInsets(top: self.margin, left: 0, bottom: 0, right: 0)
        isLayoutMarginsRelativeArrangement = true

        self.addFormElement(self.titleLabel)
    }
    ...
    open func addFormElement(_ element: UIView) {
        ...
        stackView.layoutMargins = UIEdgeInsets(top: verticalMargin, left: self.margin, bottom: verticalMargin, right: self.margin)
        stackView.isLayoutMarginsRelativeArrangement = true
        stackView.addArrangedSubview(element)
        addArrangedSubview(stackView)

        let padding = element == titleLabel ? 0 : self.margin
        self.addSeparator(leftPadding: padding)
    }
    ...
    public var title: String? {
        didSet {
            ...
            self.dynamicTypeController.observeDynamicType(for: self.titleLabel, withTextAttributes: Appearance.shared.formAttributes.sectionTitleAttributes, textStyle: .body)
