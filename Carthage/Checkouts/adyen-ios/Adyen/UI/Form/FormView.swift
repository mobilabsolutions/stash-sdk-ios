...
public class FormView: UIScrollView {
    public init() {
        ...
        self.footerStackView.addArrangedSubview(self.payButton)
        self.footerStackView.addArrangedSubview(self.payButtonSubtitle)
        addSubview(self.footerStackView)
        addSubview(self.formStackView)
    ...
    public var title: String? {
        didSet {
            if let title = title {
                self.titleLabel.attributedText = NSAttributedString(string: title, attributes: Appearance.shared.formAttributes.titleAttributes)
                if #available(iOS 11.0, *) {
                    ...
                    self.dynamicTypeController.observeDynamicType(for: self.titleLabel, withTextAttributes: Appearance.shared.formAttributes.titleAttributes, textStyle: .title1)
                }
            }
        }
    }
    ...
    public func addFormElement(_ element: UIView) {
        ...
        if element is FormSection {
            self.formStackView.addArrangedSubview(element)
            return
        }
        ...
        stackView.layoutMargins = UIEdgeInsets(top: 0, left: self.margin, bottom: 0, right: self.margin)
        stackView.isLayoutMarginsRelativeArrangement = true
        stackView.addArrangedSubview(element)
        self.formStackView.addArrangedSubview(stackView)
    ...
    public func addFooterElement(_ element: UIView) {
        self.footerStackView.addArrangedSubview(element)
    }
    ...
    private func addSeparator() {
        ...
        self.formStackView.addArrangedSubview(separator)
