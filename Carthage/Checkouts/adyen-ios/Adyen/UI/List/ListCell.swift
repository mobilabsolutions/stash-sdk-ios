...
internal final class ListCell: UITableViewCell {
    ...
    internal var isEnabled = true {
        didSet {
            let opacity: Float = self.isEnabled ? 1.0 : 0.3
    ...
    internal var disclosureIndicatorColor: UIColor? {
        didSet {
            self.disclosureImageView.tintColor = self.disclosureIndicatorColor

            let renderingMode: UIImage.RenderingMode = self.disclosureIndicatorColor == nil ? .alwaysOriginal : .alwaysTemplate
            ...
            if self.disclosureImageView.image?.renderingMode != renderingMode {
                let disclosure = self.disclosureImageView.image?.withRenderingMode(renderingMode)
                self.disclosureImageView.image = disclosure
            }
        }
    }
    ...
    private func configureConstraints() {
        ...
        let heightConstraint = self.itemView.heightAnchor.constraint(greaterThanOrEqualToConstant: 40.0)
