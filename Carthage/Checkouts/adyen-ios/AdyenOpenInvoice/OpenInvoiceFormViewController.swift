...
internal class OpenInvoiceFormViewController: FormViewController {
    ...
    var paymentMethod: PaymentMethod? {
        didSet {
            ...
            if paymentMethod.configuration?["shopperInfoSSNLookupUrl"] != nil {
                ...
                self.personalDetailsVisibility = details.personalDetails?.configuration.fieldVisibility ?? .editable
                self.billingAddressVisibility = details.billingAddress?.configuration.fieldVisibility ?? .editable
                self.deliveryAddressVisibility = details.deliveryAddress?.configuration.fieldVisibility ?? .editable
            }
        }
    }
    ...
    private var shouldShowSSNField: Bool {
        if case let .fieldSet(details)? = self.paymentMethod?.details.personalDetails?.inputType {
    ...
    @objc private func didTouchMoreInformationButton() {
        let countryCode = self.paymentSession?.payment.countryCode == "DE" ? "de" : "en"
