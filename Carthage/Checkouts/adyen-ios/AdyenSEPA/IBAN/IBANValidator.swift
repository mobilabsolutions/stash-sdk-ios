...
internal class IBANValidator: Validator {
    ...
    func isMaxLength(_ string: String) -> Bool {
        ...
        return self.sanitize(string).count >= 32
    }
    ...
    func isValid(_ string: String) -> Bool {
        let canonicalizedString = self.canonicalize(string)
        ...
        let rearrangedString = self.rearrange(canonicalizedString)
        let numerifiedString = self.numerify(rearrangedString)
    ...
    func format(_ string: String) -> String {
        let string = self.canonicalize(string)
    ...
    func mod97(_ string: String) -> Int? {
        ...
        guard string.count < 9 else {
            ...
            return self.mod97(String(lowerSubstringModulus) + upperSubstring)
        }
