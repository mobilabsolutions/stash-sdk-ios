...
internal class TestCase: XCTestCase {
    ...
    override func setUp() {
        ...
        let table = self.app.tables.first
    ...
    internal func selectPaymentMethod(_ name: String? = nil) {
        self.app.buttons["change-payment-method-button"].tap()

        if let name = name {
            let table = self.app.tables.last
            table.cells[name].tap()
        }
    }

    internal func dismissSuccessAlert() {
        let alert = self.app.alerts["Payment successful"]
        self.waitForElementToAppear(alert)
        alert.buttons.first.tap()
    }

    internal func dismissFailureAlert() {
        let alert = self.app.alerts["Payment failed"]
        self.waitForElementToAppear(alert)
        alert.buttons.first.tap()
    }
