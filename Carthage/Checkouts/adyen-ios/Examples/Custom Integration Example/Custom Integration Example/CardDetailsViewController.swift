...
class CardDetailsViewController: PaymentDetailsViewController, UITextFieldDelegate {
    ...
    override init(withPaymentMethod method: PaymentMethod, paymentController: PaymentController) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.adjustLayoutForKeyboard), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
    }
    ...
    override func submit() {
        ...
        if let name = validatedName(nameTextField.text),
            ...
            PaymentRequestManager.shared.setCardDetailsForCurrentRequest(name: name, number: number, expiryDate: expiryDate, cvc: cvc, shouldSave: self.saveSwitch.isOn)
    ...
    override func motionEnded(_ motion: UIEventSubtype, with _: UIEvent?) {
        if motion == .motionShake {
            self.populateFieldsWithFakeData()
        }
    }
    ...
    private func populateFieldsWithFakeData() {
        self.nameTextField.text = "Checkout Shopper"
        self.cardNumberTextField.text = "5555444433331111"
        self.expiryTextField.text = "08/18"
        self.cvcTextField.text = "737"
    }

    private func populateForm() {
        formScrollView.addSubview(self.nameTextFieldContainer)
        formScrollView.addSubview(self.cardNumberTextFieldContainer)
        formScrollView.addSubview(self.expiryTextFieldContainer)
        formScrollView.addSubview(self.cvcTextFieldContainer)
        formScrollView.addSubview(self.saveMethodSwitchContainerView)
    }

    private func layoutForm() {
        let availableHeight: CGFloat = view.bounds.height - self.keyboardHeightOnScreen - navigationBar.bounds.height - submitButton.bounds.height
        let fieldHeight: CGFloat = availableHeight / 4

        var frame = self.nameTextFieldContainer.frame
        frame.size.height = fieldHeight
        self.nameTextFieldContainer.frame = frame

        frame = self.nameTextFieldContainer.frame
        frame.origin.y = self.nameTextFieldContainer.frame.maxY
        self.cardNumberTextFieldContainer.frame = frame

        frame.origin.y = self.cardNumberTextFieldContainer.frame.maxY
        frame.size.width = view.bounds.size.width / 2.0
        self.expiryTextFieldContainer.frame = frame

        frame.origin.x = frame.maxX
        self.cvcTextFieldContainer.frame = frame

        frame = self.saveMethodSwitchContainerView.frame
        frame.size.height = fieldHeight
        frame.origin.y = self.cvcTextFieldContainer.frame.maxY
        self.saveMethodSwitchContainerView.frame = frame

        frame = self.saveSwitch.frame
        frame.origin.y = fieldHeight / 2 - frame.size.height / 2
        self.saveSwitch.frame = frame

        frame = submitButton.frame
        frame.origin.y = self.saveMethodSwitchContainerView.frame.maxY
    ...
    @objc private func adjustLayoutForKeyboard(notification: NSNotification) {
        if let frame = notification.userInfo?[UIKeyboardFrameEndUserInfoKey] as? CGRect {
            self.keyboardHeightOnScreen = frame.height
            self.layoutForm()
        }
    }
}
