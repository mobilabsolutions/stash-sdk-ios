...
class PaymentRequestManager {
    ...
    func select(paymentMethod: PaymentMethod) {
        ...
        let card = CardEncryptor.Card(
            number: self.cardDetails?.number,
            securityCode: self.cardDetails?.cvc,
            expiryMonth: self.cardDetails?.expiryMonth,
            expiryYear: self.cardDetails?.expiryYear
        )

        let encryptedCard = CardEncryptor.encryptedCard(for: card, publicKey: publicKey, generationDate: generationDate)

        var method = paymentMethod
        method.details.cardholderName?.value = self.cardDetails?.name
        ...
        self.paymentMethodCompletion?(method)
    }
    ...
    func setCardDetailsForCurrentRequest(name: String, number: String, expiryDate: String, cvc: String, shouldSave: Bool) {
        ...
        self.cardDetails = CardDetails(name: name, number: number, expiryMonth: monthString, expiryYear: yearString, cvc: cvc, shouldStoreDetails: shouldSave)
    }
