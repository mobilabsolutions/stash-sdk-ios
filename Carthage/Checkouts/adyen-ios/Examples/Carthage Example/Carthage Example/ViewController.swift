...
class ViewController: UIViewController {
    ...
    @IBAction private func startCheckout(_: Any) {
        self.checkoutController.start()
    }

    private func presentSuccessAlertController() {
        self.presentAlertController(with: "Payment Successful")
    }

    private func presentFailureAlertController() {
        self.presentAlertController(with: "Payment Failed")
    }
...
extension ViewController: CheckoutControllerDelegate {
    internal func requestPaymentSession(withToken token: String, for _: CheckoutController, responseHandler: @escaping (String) -> Void) {
    ...
    internal func didFinish(with result: Result<PaymentResult>, for _: CheckoutController) {
        var isSuccess = false
